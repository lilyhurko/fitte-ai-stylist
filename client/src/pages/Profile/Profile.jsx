  ...
  try {
    ...
    if (response.ok) {
      const goraFormularza = {
        ...
        styleTags: typeof data.styleTags === 'string' ? JSON.parse(data.styleTags) : (data.styleTags || [])
      };
      
      setFormData(goraFormularza);

      const storedUser = JSON.parse(sessionStorage.getItem("fitte_user") || "{}");
      if (storedUser.name !== goraFormularza.firstName || storedUser.gender !== goraFormularza.gender) {
...
const handleSaveProfile = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      setProfileMessage({ type: 'error', text: data.error || 'Wystąpił błąd.' });
    }
  } catch (error) {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPasswordMessage({ type: 'error', text: data.error || 'Błąd zmiany hasła.' });
      }
    } catch (error) {
  ...
  return (
    <main className="profile-container p-12 bg-fitte-cream min-h-screen">
      <header className="mb-8">
        ...
        <p className="text-fitte-brown-light mt-2">Zarządzaj swoimi danymi i bezpieczeństwem konta.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl">
        <section className="bg-white rounded-[40px] p-10 border border-fitte-sand shadow-sm flex flex-col justify-between">
          <div className="space-y-6">
            <h3 className="font-playfair text-xl mb-4 text-fitte-brown-dark">Dane osobowe</h3>
            
            <div className="grid grid-cols-1 gap-4">
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-bold tracking-widest text-fitte-brown-dark">IMIĘ</label>
                <input 
                  type="text" 
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-bold tracking-widest text-fitte-brown-dark">ADRES E-MAIL</label>
                <input 
                  type="email" 
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-bold tracking-widest text-fitte-brown-dark">PŁEĆ (DLA AI)</label>
                ...
                {["Kobieta", "Mężczyzna", "Inna"].map(g => (
                  <button 
                    ...
                    className={`px-5 py-2 rounded-full border text-xs font-medium transition-all ${
                      formData.gender === g ? 'bg-fitte-brown-dark text-white border-fitte-brown-dark' : 'bg-transparent border-fitte-sand text-fitte-brown-dark'
                    }`}
                  >
                    {g}
                  </button>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-bold tracking-widest text-fitte-brown-dark">PREFEROWANY STYL</label>
              <div className="flex flex-wrap gap-2 mt-1">
                {formData.styleTags.length > 0 ? (
                  ...
                  <span className="text-xs text-gray-400 italic">Brak tagów. Wypełnij quiz.</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mt-6">
            {profileMessage.text && (
              <div className={`flex items-center gap-2 text-xs mb-4 ${profileMessage.type === 'success' ? 'text-green-600' : 'text-red-500'}`}>
        ...
        <section className="bg-white rounded-[40px] p-10 border border-fitte-sand shadow-sm flex flex-col justify-between">
          <div className="space-y-5">
            <h3 className="font-playfair text-xl mb-4 text-fitte-brown-dark">Bezpieczeństwo</h3>
            
            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-bold tracking-widest text-fitte-brown-dark">OBECNE HASŁO</label>
              <input 
                type="password" 
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-bold tracking-widest text-fitte-brown-dark">NOWE HASŁO</label>
              <input 
                type="password" 
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-bold tracking-widest text-fitte-brown-dark">POWTÓRZ NOWE HASŁO</label>
              <input 
                type="password" 
          ...
          <div className="mt-6">
            {passwordMessage.text && (
              <div className={`flex items-center gap-2 text-xs mb-4 ${passwordMessage.type === 'success' ? 'text-green-600' : 'text-red-500'}`}>
            ...
            <button 
              ...
              disabled={loadingPassword || !passwordData.currentPassword || !passwordData.newPassword}
