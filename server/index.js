...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.password) return res.status(400).json({ error: "Konto wymaga zresetowania hasła." });
...
const PORT = 5001;
const server = app.listen(PORT, () => console.log(`🚀 Serwer Fitte działa na porcie ${PORT}`));
